...
*
********************************************************************/
bool CConnTree::Initialize(itcv::BoundBox rcRange)
{
    FreeMem() ;
...
********************************************************************/
int n1[4000], n2[4000];
bool CConnTree::IsLeftConnected ( int pNode1, int pNode2 )
{
    int f1, f2, i, j, k ;
    int *p1, *p2, *tp ;
...
*			
********************************************************************/
bool CConnTree::IsRightConnected ( int pNode1, int pNode2 )
{
    int f1, f2, i, j, k ;
    int *p1, *p2, *tp ;
    int x1 = m_pNode[pNode1].v.x & 0x1fffffff ;
...
*			
********************************************************************/
bool CConnTree::IsConnected ( int pNode1, int pNode2 )
{
    int x1 = m_pNode[pNode1].v.x & 0x1fffffff ;
    int x2 = m_pNode[pNode2].v.x & 0x1fffffff ;
...
*			
********************************************************************/
bool CConnTree::AllocNewNodes(int nodeNums)
{
    nodeNums = (nodeNums == 0 ? 5000 : nodeNums);
	ConnNode *pNewNodes = (ConnNode *)realloc(m_pNode, (m_nMaxNodes + nodeNums)*sizeof(ConnNode));   //the max node is 5000;
    ...
    else
    {
        m_pNode = pNewNodes ;
		int i;
        for ( i=m_nMaxNodes ; i<m_nMaxNodes+nodeNums ; i++ )
        {
...
int CConnTree::GetAllConnComps()
{
	...
	if(m_pFlag != NULL)
		free(m_pFlag);
	if ( (m_pFlag=(unsigned char *)malloc(m_pEmptHead*sizeof(unsigned char))) == (unsigned char *)NULL )
			return -1 ;
	memset ( m_pFlag, 0, m_pEmptHead*sizeof(unsigned char) ) ;
...
int CConnTree::GetConnComp ( ConnComp &cc, int nInitial)
{
	...
	if ( (pFlag=(unsigned char *)malloc(m_pEmptHead*sizeof(unsigned char))) == (unsigned char *)NULL )
			return -1 ;
	memset ( pFlag, 0, m_pEmptHead*sizeof(unsigned char) ) ;
	if(m_pStack == NULL)
...
*
********************************************************************/
int CConnTree::GetConnComp ( ConnComp &cc, int nInitial, unsigned char *pFlag)
{
    cc.nInitialNode = nInitial ;
    cc.rcBound.left = cc.rcBound.right = m_pNode[nInitial].v.x & 0x1fffffff ;
    cc.rcBound.top = m_pNode[nInitial].v.yvs ;
